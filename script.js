const RATES = {
    ...
    color: {
        ...
        "170": { "A6": 1.16, "A5": 1.9180, "A4": 3.835 }
    },
    cover: {
        "170": { "A6": 6.00, "A5": 9.00, "A4": 18.00 },
        "250": { "A6": 6.00, "A5": 9.00, "A4": 18.00 },
        "300": { "A6": 6.00, "A5": 9.00, "A4": 18.00 }
    },
    binding: {
    ...
    design: {
        cover: 2500, publishing: 3500, customLayout: { "A6": 175, "A5": 350, "A4": 450 },
...
function renderDynamicInputs() {
    ...
    checkedBoxes.forEach(type => {
        ...
        if (checkedBoxes.length === 1 && existingVal === "") existingVal = "100"; // Default starting value
        newHtml += `<div class="dynamic-pg-input">
...
function calculateEngine() {
    if (document.querySelectorAll('.sub-print-input').length === 0) renderDynamicInputs();
    ...
    document.querySelectorAll('.sub-print-input').forEach(inp => {
        ...
        if (pgs > 0) breakdownHtmlBlocks.push(`<div class="bd-row"><span>└ ${pgs}x ${t}</span><span>₹${blockCost.toFixed(2)}</span></div>`);
    });
    ...
    document.getElementById('bdScrollArea').innerHTML = `
        <div class="breakdown-header"><strong>Cost Breakdown (Per Book)</strong></div>
        <div class="bd-row" style="font-weight:600;"><span>Inner Print Total</span><span>₹${innerTotalPerBook.toFixed(2)}</span></div>
        ...
        ${emergency ? `<div class="bd-row" style="color:var(--primary);"><span>Emergency Surcharge</span><span>+₹${(mfgSubtotal * 0.35).toFixed(2)}</span></div>` : ''}
        <div class="bd-row" style="border-top: 1px solid var(--border); margin-top:10px; padding-top:10px; font-weight:800;">
            <span>Final Unit Rate</span><span>₹${perBookFinal.toFixed(2)}</span>
        </div>
        <div class="bd-row" style="margin-top:15px;"><span>Pre-Press Services</span><span>₹${totalDesign.toLocaleString('en-IN')}</span></div>
    `;
