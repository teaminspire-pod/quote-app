// --- MASTER RATE & POLICY DICTIONARY (STRICT STRUCTURE) ---
const RATES = {
    "Black & White": {
        "Natural Shade (NS)": {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadDraft();
    attachListeners();
    updatePrintInputVisibility();
    handlePaperLogic();
    triggerDebounce();
});

// Precision rounding for real-world currency math
...
// Quiet Validation - Only snaps value on Blur
function forceValidationOnBlur() {
    ...
    let displayQty = parseInt(displayQtyText);
    ...
    if (displayQty !== calcQty) {
        ...
        showToast(`Quantity locked to strict factory multiple.`);
    }

    updateQtyHint(sizeInput.value); 
    triggerDebounce();
    saveDraft();
}
...
function attachListeners() {
    ...
    // Engine listens to typing but does NOT force value replacement (keyboard stable)
    qtyInput.addEventListener('input', triggerDebounce);
    ...
    form.addEventListener('input', (e) => {
        if(e.target.id === 'bookQty' || e.target.classList.contains('sub-print-input')) return; 
        handlePaperLogic();
        triggerDebounce();
        saveDraft();
    });
    ...
    const toggleDesignEl = document.getElementById('toggleDesign');
    if (toggleDesignEl) {
        toggleDesignEl.addEventListener('change', (e) => {
            const drawer = document.getElementById('designDrawer');
            if(drawer) drawer.classList.toggle('open', e.target.checked);
        });
    }
...
function toggleDimFormat() {
    ...
    if(btn) btn.innerText = `Unit: ${currentUnit}`;
    triggerSizeHintAnimation();
}
...
function handlePaperLogic() {
    try {
        ...
        function show(el) { if (el) el.classList.remove('hidden-element'); }
        function hide(el) { if (el) el.classList.add('hidden-element'); }
        ...
        if (isMultiColorSelected) {
            ...
            if (typeArtInput && typeArtInput.checked) {
                const nsType = document.getElementById('typeNS');
                if(nsType) nsType.checked = true;
            }
        }
        ...
        const bwLabel = document.getElementById('lbl_print_bw');
        const plainLabel = document.getElementById('lbl_print_pl');

        if (typeArtInput && typeArtInput.checked) {
            let changed = false;
            if (multiCb && !multiCb.checked) { multiCb.checked = true; changed = true; }
            if (bwCb) { bwCb.checked = false; bwCb.disabled = true; hide(bwLabel); changed = true; }
            if (plainCb) { plainCb.checked = false; plainCb.disabled = true; hide(plainLabel); changed = true; }
            if(changed) updatePrintInputVisibility(); 
        } else {
            if (bwCb) { bwCb.disabled = false; show(bwLabel); }
            if (plainCb) { plainCb.disabled = false; show(plainLabel); }
        ...
        if (innerPages > 0 && innerPages < 70) {
            ...
            if ((cover170Input && cover170Input.checked) || (cover300Input && cover300Input.checked)) {
                const c250 = document.getElementById('cover250');
                if(c250) c250.checked = true;
            }
        } else {
...
// ZERO-INJECTION RENDERER: Toggles visibility via CSS, Never deletes DOM.
// Wipe state (inp.value = '') on hidden inputs to prevent silent mathematical corruption.
function updatePrintInputVisibility() {
    const types = [
        { name: "Black & White", code: "bw" },
        { name: "Multi Color", code: "mc" },
        { name: "Plain (without print)", code: "pl" }
    ];
    
    let activeCount = 0;
    
    types.forEach(t => {
        const wrapper = document.getElementById("wrapper_" + t.code);
        const input = document.getElementById("subpg_" + t.code);
        const cb = document.querySelector(`.print-type-cb[value="${t.name}"]`);
        
        if (cb && cb.checked && !cb.disabled) {
            if(wrapper) wrapper.classList.remove('hidden-element');
            activeCount++;
        } else {
            if(wrapper) wrapper.classList.add('hidden-element');
            // Force state wipe on hidden inputs to prevent calculation corruption
            if(input) input.value = ''; 
        }
    });
    
    const msg = document.getElementById('emptyPrintSelectionMsg');
    if (activeCount === 0) {
        if(msg) msg.classList.remove('hidden-element');
    } else {
        if(msg) msg.classList.add('hidden-element');
    }
}
...
function getTotalInnerPages() {
    let total = 0;
    document.querySelectorAll('.sub-print-input').forEach(inp => {
        // Strict guard: only calculate values from inputs currently visible in UI
        if(inp.closest('.print-input-wrapper') && !inp.closest('.print-input-wrapper').classList.contains('hidden-element')) {
...
function lockState(isLocked, isOffset = false, msg = "") {
    ...
    const gtLabels = [document.getElementById('gtLabelMob')];
...
// Strictly maps 4 parameters against the new expanded dictionary
function getPageRate(printType, gsm, size, paperType) {
    if(RATES[printType] && RATES[printType][paperType] && RATES[printType][paperType][gsm] && RATES[printType][paperType][gsm][size]){
        return RATES[printType][paperType][gsm][size];
    }
    return 0;
}
...
function calculateEngine() {
    ...
    // BUSINESS RULE GATEKEEPERS
    if (qty > 1000) return lockState(true, true);
    if (innerPages > 2000) return lockState(true, false, "Maximum allowed inner pages is 2000.");
    ...
    const bindingInput = document.querySelector('input[name="binding"]:checked');
    
    if (!paperGsmInput || !paperTypeInput || !bindingInput) return;

    const paperGsm = paperGsmInput.value;
    const paperType = paperTypeInput.value;
    const bindingChoice = bindingInput.value;
    const emergencyEl = document.getElementById('emergencyCharge');
    ...
    function getCascadeLine(label, val, overrideCurrency = false) {
        const displayVal = overrideCurrency ? val : `₹${val.toFixed(2)}`;
        const line = `<div class="bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">${label}</span><span class="bd-value">${displayVal}</span></div>`;
    ...
    // Base Calculation: Subtract from base cover cost before multiplying by qty.
    if (laminationInput && laminationInput.value === "No Lamination") {
        coverCostPerBook = preciseRound(coverCostPerBook - 0.50); 
    }

    // BASE MANUFACTURING WITHOUT BINDING (Inner + Cover)
    const totalInnerAndCover = preciseRound((innerTotalPerBook + coverCostPerBook) * qty);
    
    // --- PERFECT BINDING CRITICAL MINIMUM CHARGE LOGIC ---
    let totalBindingCost = 0;
    let bindingLabel = "Binding";
    let isMinimumBindingApplied = false;
    let bindingPerBookFallback = 0; // Fallback unit rate for breakdown display tracking
    
    if (bindingChoice === 'Hard Binding') {
        let hardRate = 0;
        for (let tier of RATES.hardBindingTiers) {
            if (qty <= tier.max) { hardRate = tier.rate; break; }
        }
        totalBindingCost = preciseRound(qty * hardRate);
        bindingPerBookFallback = hardRate;
    } else if (bindingChoice === 'Perfect Binding') {
        let perfectRate = RATES.binding['Perfect Binding']?.[size] ?? 0;
        let bindingCalculated = preciseRound(qty * perfectRate);
        
        let minCharge = 0;
        if (qty < 100) {
            if (innerPages >= 1 && innerPages <= 500) minCharge = 500;
            else if (innerPages >= 501 && innerPages <= 1000) minCharge = 600;
        }

        if (bindingCalculated < minCharge && minCharge > 0) {
            // STRICT FIXED TOTAL - Bypass per-book multiplier
            totalBindingCost = minCharge;
            bindingLabel = `Binding (Minimum Charge Applied)`;
            isMinimumBindingApplied = true;
        } else {
            // Normal calculation
            totalBindingCost = bindingCalculated;
            bindingPerBookFallback = perfectRate;
        }
    } else {
        // Spiral Binding & Centre Stapling
        let standardRate = RATES.binding[bindingChoice]?.[size] ?? 0;
        totalBindingCost = preciseRound(qty * standardRate);
        bindingPerBookFallback = standardRate;
    }

    // FINAL MANUFACTURING COMPOSITION
    const baseMfgTotal = preciseRound(totalInnerAndCover + totalBindingCost);
    const emergencySurcharge = emergency ? preciseRound(baseMfgTotal * 0.35) : 0;
    const grandMfgTotal = preciseRound(baseMfgTotal + emergencySurcharge);
    const perBookFinal = preciseRound(grandMfgTotal / qty); // Re-derived unit rate
    ...
    if (toggleDesignCb && toggleDesignCb.checked) {
        const pubPkg = document.getElementById('pubPackage');
        if (pubPkg && pubPkg.checked) {
            totalDesign += RATES.design.publishing;
            designLinesHTML += getDesignLine('Publishing Package', RATES.design.publishing);
        }
        const cvrDsg = document.getElementById('coverDesign');
        if (cvrDsg && cvrDsg.checked) {
            totalDesign += RATES.design.cover;
            designLinesHTML += getDesignLine('Cover Design', RATES.design.cover);
        }
        
        let cPgsEl = document.getElementById('designPages');
        let cPgs = cPgsEl ? parseInt(cPgsEl.value) : 0;
        if (isNaN(cPgs)) cPgs = 0;
        if (cPgs > 0) {
        ...
        let lPgsEl = document.getElementById('layoutPages');
        let lPgs = lPgsEl ? parseInt(lPgsEl.value) : 0;
        if (isNaN(lPgs)) lPgs = 0;
        if (lPgs > 0) {
            let cost = preciseRound(lPgs * RATES.design.basicLayout);
            totalDesign += cost; designLinesHTML += getDesignLine(`Basic Layout (${lPgs})`, cost);
        }
        
        let dPgsEl = document.getElementById('dtpPages');
        let dPgs = dPgsEl ? parseInt(dPgsEl.value) : 0;
        if (isNaN(dPgs)) dPgs = 0;
        if (dPgs > 0) {
            let cost = preciseRound(dPgs * RATES.design.dtp);
            totalDesign += cost; designLinesHTML += getDesignLine(`DTP (${dPgs})`, cost);
        }
        
        let pPgsEl = document.getElementById('proofingPages');
        let pPgs = pPgsEl ? parseInt(pPgsEl.value) : 0;
        if (isNaN(pPgs)) pPgs = 0;
        if (pPgs > 0) {
    ...
    const grandTotal = preciseRound(grandMfgTotal + totalDesign);

    // CASCADE BREAKDOWN RENDERING
    let bdHtml = innerLinesHTML;
    bdHtml += getCascadeLine('Cover Stock & Lam', coverCostPerBook);
    
    // Explicit binding line mapping (handles fixed total vs per-book display cleanly)
    if (isMinimumBindingApplied) {
        bdHtml += getCascadeLine(bindingLabel, `₹${totalBindingCost.toFixed(2)}`, true);
    } else {
        bdHtml += getCascadeLine(bindingLabel, bindingPerBookFallback);
    }
    
    bdHtml += `<div class="bd-subtotal bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">Total Mfg Cost (Batch)</span><span class="bd-value">₹${baseMfgTotal.toFixed(2)}</span></div>`;
    delayIter++;
    ...
    exportData = {
        ...
        bindingLabelExport: bindingLabel,
        isMinimumBindingApplied: isMinimumBindingApplied,
        totalBindingCost: totalBindingCost,
        bindingPerBookFallback: bindingPerBookFallback,
        innerTotalPerBook: innerTotalPerBook,
        coverCostPerBook: coverCostPerBook,
...
async function processExport() {
    ...
    msg += `Binding: ${exportData.binding}\n\n`;

    if (exportData.emergency) {
        msg += `*Priority Production (+35%)*\n`;
        msg += `_(Fast-track production with guaranteed print readiness within 24 hours from order confirmation, followed by same-day dispatch. If we are unable to complete the order within the committed priority timeline, the additional emergency priority charges will be fully refundable.)_\n\n`;
    }

    msg += `*Manufacturing Breakdown:*\n`;
    msg += `• Inner Pages: ₹${exportData.innerTotalPerBook.toFixed(2)} (Per Book)\n`;
    msg += `• Cover Stock & Lam: ₹${exportData.coverCostPerBook.toFixed(2)} (Per Book)\n`;
    
    if (exportData.isMinimumBindingApplied) {
        msg += `• ${exportData.bindingLabelExport} = ₹${exportData.totalBindingCost.toFixed(2)}\n`;
    } else {
        msg += `• Binding: ₹${exportData.bindingPerBookFallback.toFixed(2)} (Per Book)\n`;
    }

    if (exportData.emergency) {
        msg += `• Priority Surcharge (+35%): +₹${exportData.emergencySurcharge.toFixed(2)} (Total Batch)\n`;
    }
    
    msg += `\n*Final Unit Rate: ₹${exportData.perBookMfgRate}/-*\n\n`;

    if (exportData.designActive && exportData.totalDesignFees > 0) {
        msg += `*Design & Publishing Services:*\n`;
        const pubPkg = document.getElementById('pubPackage');
        if (pubPkg && pubPkg.checked) msg += `• Publishing Package Included\n`;
        const cvrDsg = document.getElementById('coverDesign');
        if (cvrDsg && cvrDsg.checked) msg += `• Cover Design Generation\n`;
        
        const cPgsEl = document.getElementById('designPages');
        let cPgs = cPgsEl ? parseInt(cPgsEl.value) : 0;
        if(isNaN(cPgs)) cPgs = 0;
        
        const lPgsEl = document.getElementById('layoutPages');
        let lPgs = lPgsEl ? parseInt(lPgsEl.value) : 0;
        if(isNaN(lPgs)) lPgs = 0;
        
        const dPgsEl = document.getElementById('dtpPages');
        let dPgs = dPgsEl ? parseInt(dPgsEl.value) : 0;
        if(isNaN(dPgs)) dPgs = 0;
        
        const pPgsEl = document.getElementById('proofingPages');
        let pPgs = pPgsEl ? parseInt(pPgsEl.value) : 0;
        if(isNaN(pPgs)) pPgs = 0;
