...
function validateQuantity() {
    ...
    let qtyText = qtyInput.value;
    
    // Safety check for completely empty input
    if (qtyText.trim() === '') qtyText = '0';
    let qty = parseInt(qtyText) || 0;
...
function calculateEngine() {
    ...
    subInputs.forEach(inp => {
        ...
        innerTotalPerBook += preciseRound(pgs * rate);
    });
    ...
    // --- RENDER INTERNAL BREAKDOWN (Variables Injected + Dynamic Fade) ---
    let bdHtml = `
        <div class="bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">Inner Printing</span><span class="bd-value">₹${innerTotalPerBook.toFixed(2)}</span></div>
        `; delayIter++; bdHtml += `
        <div class="bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">Cover Stock & Lam</span><span class="bd-value">₹${coverCostPerBook.toFixed(2)}</span></div>
        `; delayIter++; bdHtml += `
        <div class="bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">Binding Style</span><span class="bd-value">₹${bindingCostPerBook.toFixed(2)}</span></div>
        `; delayIter++; bdHtml += `
        <div class="bd-subtotal bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label">Manufacturing Total</span><span class="bd-value">₹${mfgSubtotal.toFixed(2)}</span></div>
    `; delayIter++;
    ...
    bdHtml += `<div class="cascade-row" style="margin: 20px 0; border-top: 1px dashed var(--border); animation-delay: ${delayIter * 0.05}s"></div>`;
    delayIter++;
    
    bdHtml += `<div class="bd-row cascade-row" style="animation-delay: ${delayIter * 0.05}s"><span class="bd-label" style="color:var(--text-main);">Pre-Press Fees (Total)</span><span class="bd-value">₹${totalDesign.toLocaleString('en-IN')}</span></div>`;
    delayIter++;
...
function saveDraft() {
    ...
    // Overwriting the entire object prevents "ghost data" from persisting.
