...
// ZERO-INJECTION RENDERER: Toggles visibility, never deletes DOM
...
function getTotalInnerPages() {
    let total = 0;
    document.querySelectorAll('.sub-print-input').forEach(inp => {
        // Only count visible inputs to prevent state ghosting
        if(!inp.parentElement.classList.contains('hidden-element')) {
...
function calculateEngine() {
    ...
    const bindingStyle = bindingInput.value;
    ...
    if (bindingStyle === 'Hard Binding') {
        ...
        if (totalStandardBindingCost < minCharge && minCharge > 0) {
            bindingCostPerBook = preciseRound(minCharge / qty);
            bindingLabel = `Binding (Min Charge ₹${minCharge})`;
        } else {
    ...
    bdHtml += getCascadeLine(bindingLabel, bindingCostPerBook);
    ...
    exportData = {
        ...
        bindingCostPerBook: bindingCostPerBook,
...
async function processExport() {
    ...
    msg += `*Manufacturing Breakdown (Per Book):*\n`;
    msg += `• Inner Pages: ₹${exportData.innerTotalPerBook.toFixed(2)}\n`;
    msg += `• Cover Stock & Lam: ₹${exportData.coverCostPerBook.toFixed(2)}\n`;
    msg += `• ${exportData.bindingLabelExport}: ₹${exportData.bindingCostPerBook.toFixed(2)}\n`;
    if (exportData.emergency) {
        msg += `• Priority Surcharge (+35%): +₹${exportData.emergencySurcharge.toFixed(2)}\n`;
    }
