...
function handlePaperLogic() {
    try {
        ...
        // HIDE 70 GSM INNER & 170 GSM COVER BELOW 70 PAGES
        if (innerPages < 70) {
            hide(gsm70Label);
...
function updateSizeHintDirect() {
    ...
    if (sizeInput) document.getElementById('sizeHintDisplay').innerText = dimData[sizeInput.value][currentUnit];
}
...
function calculateEngine() {
    ...
    const paperGsm = paperGsmInput.value; // e.g., "70"
    ...
    if (checkedTypes.length === 1) {
        ...
        if (rate === 0) return lockState(true, false, `Invalid combination for ${t}.`);
        innerTotalPerBook = innerPages * rate;
    } else {
        let subCost = 0;
        const subInputs = document.querySelectorAll('.sub-print-input');
        
        // Auto-balance logic for exactly 2 inputs
    ...
    if (laminationInput && laminationInput.value === "No Lamination") {
        coverCostPerBook -= 0.50; // -₹0.50 per book discount
    }
    ...
    if (bindingStyle === 'Hardbinding') {
        ...
        // Tiered Minimum Binding Charge Logic
        let minCharge = 0;
    ...
    // Emergency Surcharge applies ONLY to manufacturing subtotal
