// --- MASTER RATE & POLICY DICTIONARY (VERIFIED: MAY 2026) ---
const RATES = {
    bw: {
...
function attachListeners() {
    ...
    // Engine listens to typing but does NOT force value replacement
    qtyInput.addEventListener('input', () => {
        triggerDebounce();
    });
    
    // Final lock occurs only when tapping away
...
function handlePaperLogic() {
    try {
        ...
        if (typeArtInput && typeArtInput.checked) {
            ...
            if (bwCb) { bwCb.checked = false; bwCb.disabled = true; if(bwLabel){ bwLabel.classList.add('smooth-hidden'); bwLabel.style.display = 'none'; } changed = true; }
            if (plainCb) { plainCb.checked = false; plainCb.disabled = true; if(plainLabel){ plainLabel.classList.add('smooth-hidden'); plainLabel.style.display = 'none'; } changed = true; }
            if(changed) renderPrintInputs(); 
        } else {
            if (bwCb) { bwCb.disabled = false; if(bwLabel){ bwLabel.classList.remove('smooth-hidden'); bwLabel.style.display = 'flex'; } }
            if (plainCb) { plainCb.disabled = false; if(plainLabel){ plainLabel.classList.remove('smooth-hidden'); plainLabel.style.display = 'flex'; } }
...
// Strict NaN fallback protecting the core engine
...
function calculateEngine() {
    ...
    // Secure all dynamic variables for the WhatsApp Exporter
    exportData = {
...
async function processExport() {
    ...
    // The WhatsApp Full Rate Breakdown Injection
