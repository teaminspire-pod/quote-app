...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('input[type="number"]').forEach(inp => {
        inp.addEventListener('input', () => {
            ...
            if (val < 0) val = 0;
            if (inp.id !== 'bookQty') inp.value = Math.floor(val); 
        });
    });
...
// MOQ & Auto-Rounding Logic (Fires on Blur/Change to prevent typing glitches)
...
function attachListeners() {
    // Attach blur and change for clean validation
    const qtyInput = document.getElementById('bookQty');
    ...
    form.addEventListener('input', (e) => {
        if(e.target.id === 'bookQty') {
            triggerDebounce(); // Calculate as they type without forcing multiples
            return; 
        }
    ...
    form.addEventListener('change', (e) => {
        if(e.target.id === 'bookQty') return; // Handled by validateQuantity
        if(e.target.classList.contains('print-type-cb')) {
            renderPrintInputs();
        }
        handlePaperLogic();
    ...
    document.querySelectorAll('input[name="masterSize"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            triggerDebounce();
            validateQuantity(); // Re-validate if size changes
        });
    });
}
...
function handlePaperLogic() {
    try {
        ...
        // ART PAPER MULTI-COLOR LOCK (With Desync Fix)
        if (typeArtInput && typeArtInput.checked) {
            let changed = false;
...
function calculateEngine() {
    ...
    // --- RENDER INTERNAL BREAKDOWN (Variables Injected) ---
    let bdHtml = `
        <div class="bd-row cascade-row" style="animation-delay: 0.05s"><span class="bd-label">Inner Printing</span><span class="bd-value">₹${innerTotalPerBook.toFixed(2)}</span></div>
        <div class="bd-row cascade-row" style="animation-delay: 0.1s"><span class="bd-label">Cover Stock & Lam</span><span class="bd-value">₹${coverCostPerBook.toFixed(2)}</span></div>
        <div class="bd-row cascade-row" style="animation-delay: 0.15s"><span class="bd-label">Binding Style</span><span class="bd-value">₹${bindingCostPerBook.toFixed(2)}</span></div>
        <div class="bd-subtotal bd-row cascade-row" style="animation-delay: 0.2s"><span class="bd-label">Manufacturing Total</span><span class="bd-value">₹${mfgSubtotal.toFixed(2)}</span></div>
    `;
    if (emergency) bdHtml += `<div class="bd-row bd-emergency cascade-row" style="animation-delay: 0.25s"><span class="bd-label">Priority Production (+35%)</span><span class="bd-value">+₹${emergencySurcharge.toFixed(2)}</span></div>`;
    
    bdHtml += `<div class="bd-row cascade-row" style="margin-top:12px; font-weight:800; color:var(--text-main); font-size:1.05rem; animation-delay: 0.3s"><span class="bd-label">Final Unit Rate</span><span>₹${perBookFinal.toFixed(2)}</span></div>`;
    bdHtml += `<div class="cascade-row" style="margin: 20px 0; border-top: 1px dashed var(--border); animation-delay: 0.35s"></div>`;
    bdHtml += `<div class="bd-row cascade-row" style="animation-delay: 0.4s"><span class="bd-label" style="color:var(--text-main);">Pre-Press Fees (Total)</span><span class="bd-value">₹${totalDesign.toLocaleString('en-IN')}</span></div>`;
...
// --- DRAFT CACHING (Race Condition Fixed) ---
function saveDraft() {
    const data = {};
...
function loadDraft() {
    ...
    try {
        ...
        if(data['checkedPrintTypes']) {
            ...
            renderPrintInputs(); // MUST force render BEFORE trying to inject sub-values
        }
